...
ClosedHash::ClosedHash(int size)//constructor
{
  hashSize = size;
...
void ClosedHash::setType(int typeNo)//called in main to set type of hashTable
{
  if (typeNo == 1)
  {
    quadHash = true;
...
bool ClosedHash::find(int searchValue)//find function for insert and delete
{
...
int ClosedHash::hash(int insertValue, bool quadHash, int iter) //method to determine hashing method
{
...
bool ClosedHash::deleteNode(int deleteValue) // find and remove
{
  if(!find(deleteValue))
  {
